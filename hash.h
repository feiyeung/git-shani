...
static inline void git_SHA1_Init(git_SHA_CTX *c)
{
...
static inline int git_SHA1_Update(git_SHA_CTX *c, const void *data, unsigned long len)
{
    while (len > 0)
    {
        if (c->len == 0 && len >= 64)
...
static inline int git_SHA1_Final(unsigned char *md, git_SHA_CTX *c)
{
    c->msgbuf[c->len] = 0x80;
    ++c->len;
